...
public class EnN extends AnalyseModel {
	...
	public EnN() {
		super(new Inconsistency(InconsistencyType.EnN, Severity.HIGH));
	}
	...
	public void analyse() {
		...
		for (SequenceMessage sequenceMessage : sequenceDiagram.getMessages()) {
			if (sequenceMessage.getMessageName() == null) {
				...
				InconsistencyError error = new InconsistencyError("object",
						sequenceMessage.getSender().getLifelineName(), null, errorMessage);
				this.addError(error);
			}
		}
	}
